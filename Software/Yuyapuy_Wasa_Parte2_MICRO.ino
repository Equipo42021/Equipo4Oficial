...
SoftwareSerial BT(10, 11);//Create a serial connection with TX and RX on these pins

int sum1=0; //Variables que se usarán para los cálculos
int sum2=0;
int sum3=0;
int sum4=0;
int sum5=0;
    ...
    if (BT.read()=='1')
    {
      if (Serial.available()>0) //Si hay un valor en el monitor serial...
      {
        ...
        while (a=="1") //Si el valor es 1 procedo a correr el bucle
        {
          ...
          //BT.write(analogRead(A0), analogRead(A1), analogRead(A2), analogRead(A3), analogRead(A4), analogRead(A5), media1, media2, media3, media4, media5) Se envía el vector para ser desglozado en python y distribuirlo a listas para su clasificación

          if (BT.available()) //En el caso se encuentre otro valor...
          {
            if (BT.read() != "1")
            {
        ...
        PORTD=0x00; //Apago los leds cuando se finaliza el examen
      }
